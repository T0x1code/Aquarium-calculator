...
with st.expander("Розрахунок солей для підміни", expanded=True):
    st.markdown("""
    **Як це працює:** Ви задаєте бажані параметри води (GH, KH, Ca:Mg), а система розраховує точну кількість солей.
    """)
    ...
    with col_rem2:
        ...
        # 1°dH = 7.14 мг/л CaO
        # 1°dH CaO містить 5.1 мг/л Ca
        # 1°dH MgO містить 4.3 мг/л Mg
        
        # Визначаємо внесок Ca та Mg в GH
        # GH = Ca_мгл/5.1 + Mg_мгл/4.3
        # При цьому Ca_мгл / Mg_мгл = target_ca_mg_ratio
        
        # Звідси: Mg_мгл = target_gh / (target_ca_mg_ratio/5.1 + 1/4.3)
        #         Ca_мгл = Mg_мгл * target_ca_mg_ratio
        
        mg_mgl = target_gh / (target_ca_mg_ratio / 5.1 + 1 / 4.3)
        ca_mgl = mg_mgl * target_ca_mg_ratio
        
        # Загальна кількість Ca та Mg в грамах на об'єм
    ...
    # Інструкція у окремому expander'і
    with st.expander("📖 Інструкція приготування", expanded=True):
        ...
        2. **Додайте солі** в такому порядку:
           - 🧂 **CaCl₂·2H₂O (кальцій хлорид)** — додайте першим, розмішайте до повного розчинення.
           - 🧂 **MgSO₄·7H₂O (магній сульфат)** — добре розчиняється.
           - 🧂 **CaCO₃ (кальцій карбонат)** — **Зверніть увагу!** CaCO₃ погано розчиняється у воді без подачі вуглекислого газу, тому вода стане молочно-білою. Це нормальна реакція. Вилийте цю суспензію в акваріум. Там, під дією подачі CO₂, вона поступово розчиниться, і вода стане кришталево чистою, а KH підніметься до цільових.
        
        3. **Перемішайте** до максимально можливого розчинення.
        
        4. **Виміряйте TDS** — має бути ~{predicted_tds:.0f} ppm.
        ...
        | Орієнтовний TDS | **{predicted_tds:.0f} ppm** |
        """)
    
    # Додаткове попередження про CaCO₃
    st.info("💡 **Важливо:** CaCO₃ погано розчиняється без CO₂. Не лякайтеся молочно-білого кольору води — це нормально. При подачі CO₂ в акваріумі він повністю розчиниться.")

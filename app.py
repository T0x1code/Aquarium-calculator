...
# Додана інструкція знизу
    st.divider()
    st.markdown(f"""
    ### 📝 Інструкція з приготування:
    1. **Підготовка води:** Наберіть **{c_vol:.0f} л** осмотичної води у ємність для підміни.
    2. **Розчинення:** Спочатку додайте **{mgso4_g:.3f} г MgSO₄** (Сульфат магнію) та **{cacl2_g:.3f} г CaCl₂** (Хлорид кальцію). Добре перемішайте до повного розчинення (це відбувається майже миттєво).
    3. **Внесення карбонату (KH):** Додайте **{kh_from_caco3:.3f} г CaCO₃** (Карбонат кальцію). 
       * *Зверніть увагу:* CaCO₃ погано розчиняється у воді без подачі вуглекислого газу, тому вода стане молочно-білою. Це нормальна реакція.
       * Вилийте цю суспензію в акваріум. Там, під дією подачі CO₂, вона поступово розчиниться, і вода стане кришталево чистою, а KH підніметься до цільових **{target_kh} dH**.
    """)

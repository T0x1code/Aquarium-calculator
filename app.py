...
with st.expander("Аналіз минулого періоду"):
    ...
    def calc_real_cons(tab, name, key_p, is_po4=False):
        with tab:
            ...
            p_test = c1.number_input(f"Тест {name} (початок)", value=15.0 if not is_po4 else 1.0, step=0.1, key=f"p_{key_p}")
            c_test = c2.number_input(f"Тест {name} (зараз)", value=10.0 if not is_po4 else 0.5, step=0.1, key=f"c_{key_p}")
            
            with c3:
                st.markdown("**Внесено добрив:**")
                dose_ml = st.number_input(f"мл добрива", value=0.0, step=1.0, key=f"d_ml_{key_p}")
                conc = st.number_input(f"Концентрація {name} г/л", value=50.0 if not is_po4 else 5.0, step=1.0, key=f"conc_{key_p}")
                added_mgl = (dose_ml * conc) / tank_vol if tank_vol > 0 else 0
                st.caption(f"Додано: +{added_mgl:.1f} мг/л")
            
            cl1, cl2 = st.columns(2)
            ch_l = cl1.number_input(f"Літрів підмінено", value=0.0, step=1.0, key=f"ch_l_{key_p}")
            ...
            # Формула: споживання = (початок*(1-підміна) + додано - зараз) / дні
...
with col1:
    ...
    if po4_unit == "P (фосфор)":
        po4_input = st.number_input("P (мг/л)", value=0.3, step=0.1)
...
with col2:
    ...
    ph_morning = st.number_input("pH (ранок, до CO₂)", value=7.2, step=0.1)
    ph_evening = st.number_input("pH (вечір, через 2-3 год CO₂)", value=6.8, step=0.1)

...
st.title("Toxicode Aquarium System V9.3 — C:N:P:K + Control")
...
with col_rec1:
    ...
    if delta_no3 > 0:
        st.warning(f"NO3 deficit: {delta_no3:.1f} mg/L")
        st.info(f"Add +{change_n_ml:.1f} mL/day")
    elif delta_no3 < 0:
        st.warning(f"NO3 excess: {abs(delta_no3):.1f} mg/L")
        st.info(f"Reduce by {reduce_n_ml:.1f} mL/day")
    else:
        st.success("NO3 normal")

    st.metric("New recommended dose", f"{max(0, new_dose_n):.1f} mL/day")
...
with col_rec2:
    ...
    if delta_po4 > 0:
        st.warning(f"PO4 deficit: {delta_po4:.2f} mg/L")
        st.info(f"Add +{change_p_ml:.2f} mL/day")
    elif delta_po4 < 0:
        st.warning(f"PO4 excess: {abs(delta_po4):.2f} mg/L")
        st.info(f"Reduce by {reduce_p_ml:.2f} mL/day")
    else:
        st.success("PO4 normal")

    st.metric("New recommended dose", f"{max(0, new_dose_p):.2f} mL/day")
...
with col_rec3:
    ...
    if delta_k > 0:
        st.warning(f"K deficit: {delta_k:.1f} mg/L")
        st.info(f"Add +{change_k_ml:.1f} mL/day")
    elif delta_k < 0:
        st.warning(f"K excess: {abs(delta_k):.1f} mg/L")
        st.info(f"Reduce by {reduce_k_ml:.1f} mL/day")
    else:
        st.success("K normal")

    st.metric("New recommended dose", f"{max(0, new_dose_k):.1f} mL/day")

st.caption("How to read: if deficit - add, if excess - reduce. Change dosage gradually.")
...
N: {current_dose_n_ml:.1f} -> {max(0, new_dose_n):.1f} mL/day
P: {current_dose_p_ml:.2f} -> {max(0, new_dose_p):.2f} mL/day
K: {current_dose_k_ml:.1f} -> {max(0, new_dose_k):.1f} mL/day
====================================="""
...
st.caption("Toxicode V9.3 | C:N:P:K Control | Dynamic dose correction")

...
with st.expander("Розрахунок ремінералізації для підміни", expanded=True):
    ...
    if rem_mode == "Ручний розрахунок (солі)":
        ...
        # Розрахунок солей (базова формула)
        ...
        **Salty Shrimp Bee Shrimp Mineral GH+**
        ...
        ss_dose = target_gh_only * c_vol * 1.5 / 10  # 1.5г на 10л на 1°dH
        ...
        **Aquayer Ремінерал GH/KH+**
        ...
        q_dose = target_gh_kh * c_vol * 2 / 10
        st.success(f"**Додай {q_dose:.1f} г Aquayer GH/KH+ на {c_vol:.0f} л води**")
